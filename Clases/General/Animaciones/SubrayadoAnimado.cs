...
{
    /// <summary>
    /// Diccionario para manejar el estado de la animación de cada control.
    /// </summary>
    private static readonly Dictionary<object, (Timer Timer, int LineWidth, bool IsAnimating)> Estados = new();

    /// <summary>
    /// Método para dibujar la línea de subrayado en un control.
    /// </summary>
    ...
    /// <summary>
    /// Inicia la animación de subrayado en un control.
    /// </summary>
    public static void Iniciar(object target, int incrementoBase = 5)
    {
        if (!Estados.ContainsKey(target))
    ...
    /// <summary>
    /// Detiene la animación de subrayado en un control.
    /// </summary>
    public static void Detener(object target)
    {
        if (Estados.TryGetValue(target, out var estado))
    ...
    /// <summary>
    /// Aplica la animación a un botón, agregando eventos para el hover.
    /// </summary>
    public static void AplicarABoton(Button button, Color color)
    {
        button.Paint += (s, e) => Aplicar(button, e.Graphics, color, 3);
        button.MouseEnter += (s, e) => Iniciar(button);
        button.MouseLeave += (s, e) => Detener(button);
    }
}
